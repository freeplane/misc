...
class visorFreeplane.Main {
		...
		static function redefineRightMenu(){
			...
			mycm.onSelect = visorFreeplane.Main.copyInfoNodeOver;
			var copy=new ContextMenuItem("move back",visorFreeplane.Main.backward);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("move forward",visorFreeplane.Main.forward);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("copy to clipboard",visorFreeplane.Main.getNodeText);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("unfold all from Node",visorFreeplane.Main.unfoldAllFromNode);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("fold all from Node",visorFreeplane.Main.foldAllFromNode);
			mycm.customItems.push(copy);
			if(Browser.flashVersion>7){
				copy=new ContextMenuItem("gen shots for linked maps",visorFreeplane.Main.genShotsForLinkedMaps);
				mycm.customItems.push(copy);
			}
			copy=new ContextMenuItem("FREEPLANE BROWSER v1.1b",visorFreeplane.Main.nada);
			mycm.customItems.push(copy);
			mycm.hideBuiltInItems();
			_root.menu = mycm;
		}
		...
		static public function run():Boolean
	   {
		   ...
		   trace("Starting flash Freeplane Browser");
