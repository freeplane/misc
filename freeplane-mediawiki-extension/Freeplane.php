...
function renderMindmap($input)
{
    ...
    if ($mm_type === "flash") {
        ...
        if (strcasecmp($mm_target, "embedded") == 0) {
            $flashContentCounter++;
            require_once("Freeplane/flashwindowFunction.php");
            $output = getMindMapFlashOutput($mm_title, $params, $flashContentCounter, $mm_height, "$wgScriptPath/extensions/Freeplane/");
        } else if (strcasecmp($mm_target, "link") == 0) {
            $Formcounter++;
            $ref = "$wgScriptPath/extensions/Freeplane/flashwindow.php?";
        } else {
        ...
        if (strcasecmp($mm_target, "embedded") == 0) {
            require_once("Freeplane/appletwindowFunction.php");
            $output = getMindMapAppletOutput($mm_title, $params, $mm_height, "$wgScriptPath/extensions/Freeplane/");
        } else if (strcasecmp($mm_target, "link") == 0) {
            $ref = "$wgScriptPath/extensions/Freeplane/appletwindow.php?";
        } else {
            $output = MindmapHelp($url);
        }
    } else {
